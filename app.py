...
def search_password():
    ...
    if len(website) == 0:
        ...
        try:
            with open("data.json", mode="r") as old_data:
...
def database(new_entry):
    try:
        with open("data.json", mode="r") as old_entry:
            password_data = json.load(old_entry)
    except (FileNotFoundError, json.decoder.JSONDecodeError):
        with open("data.json", mode="w") as new_password_entry:
            json.dump(new_entry, new_password_entry, indent=4)
    else:
        password_data.update(new_entry)
        with open("data.json", mode="w") as old_password_entry:
